...
class Round(models.Model):
    ...
    major_case = models.ForeignKey(MajorCase, on_delete=models.CASCADE, verbose_name=_(
        'major case'), blank=True, null=True)
...
class Puzzle(models.Model):
    ...
    round = models.ForeignKey(
        Round, on_delete=models.CASCADE, verbose_name=_('round'))
    order = models.IntegerField(default=0, verbose_name=_('Order'))
...
class Team(models.Model):
    ...
    brown_members = models.BooleanField(default=False, verbose_name=_(
        'Any Brown community members on the team?'), help_text=_('(Undergraduates, Graduates, Faculty, or Alumni)'))
    brown_affiliation_desc = models.CharField(default="", max_length=200, verbose_name=_(
        'For each member, please describe their affiliation to Brown/RISD (if applicable)'))
    in_person_sat = models.IntegerField(
        default=0, verbose_name=_('number person on Saturday, April 15th'))
    in_person_sun = models.IntegerField(
        default=0, verbose_name=_('number person on Sunday, April 16th?'))
    classroom_need = models.BooleanField(default=False, verbose_name=_('Do you want to request a classroom to hunt in?'), help_text=_(
        'Our availability will be limited, so please do not request one if you can make alternate plans.'))
    location = models.CharField(default=_('NO LOCATION'), max_length=200, verbose_name=_(
        'Location during hunt'), help_text=_('(e.g: Hegeman Common Room, Barus and Holley Room ###, Zoom, Discord, etc.)'))
    phone_number = models.CharField(
        default=_('-1'), max_length=200, verbose_name=_('Phone number'))

    merge_out = models.BooleanField(verbose_name=_(
        'Is the team interested in joining up with a larger team for the hunt.'), default=False)
    merge_out_preferences = models.CharField(default="", max_length=200, verbose_name=_(
        'Merge out with larger team preferences'), help_text=_('(e.g. size, age-range, in-person vs remote, etc.)'))
    merge_in = models.BooleanField(verbose_name=_(
        'Is the team interested in taking on lone-solvers.'), default=False)
    merge_in_preferences = models.CharField(default="", max_length=200, verbose_name=_('Lone solver merge in preferences'), help_text=_(
        '(e.g. size, age-range, in-person vs remote, etc.). DO NOT GIVE A TEAM MORE THAN THE NUMBER OF MEMBERS THEY ARE ASKING FOR!'))
    ...
    def compute_unlocks(context):
        ...
        for puzzle in context.all_puzzles:
            ...
            if 0 <= puzzle.unlock_hours and (
                ...
                unlock_time = context.start_time + \
                    datetime.timedelta(hours=puzzle.unlock_hours)
            ...
            if context.is_admin or context.hunt_is_over:
                ...
                if puzzle.id in context.team.db_unlocks:
                    ...
                    unlocks.append(Team.unlock_puzzle(
                        context, puzzle, unlocked_at))
            if unlocked_at:
...
class MinorCaseIncoming(models.Model):
    ...
    incoming_datetime = models.DateTimeField(
        verbose_name=_('Incoming datetime'))
...
class Erratum(models.Model):
    ...
    puzzle = models.ForeignKey(
        Puzzle, null=True, blank=True, on_delete=models.CASCADE, verbose_name=_('puzzle'))
    ...
    def get_emails(self):
        teams = PuzzleUnlock.objects.filter(puzzle=self.puzzle).exclude(
            view_datetime=None).values_list('team_id', flat=True)
...
class VoiceRecording(models.Model):
    ...
    audio_url = models.URLField(verbose_name=_('Audio URL'))
    ...
    class Meta:
        indexes = [GinIndex(fields=['transcript'], opclasses=[
                            'gin_trgm_ops'], name='transcript_trgm_idx')]
