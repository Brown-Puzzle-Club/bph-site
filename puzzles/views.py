...
# NOTE: further views are not used, but likely very helpful in reimplementing features of gph-site.
...
#     solution_access = (
#         request.context.is_superuser or request.context.hunt_solutions_open
#     )
...
def guess_csv(request):
    ...
    fname = "bph_guesslog_{}.csv".format(request.context.now.strftime("%Y%m%dT%H%M%S"))
...
def hint_csv(request):
    response = HttpResponse(content_type="text/csv")
    fname = "bph_hintlog_{}.csv".format(request.context.now.strftime("%Y%m%dT%H%M%S"))
    response["Content-Disposition"] = 'attachment; filename="{}"'.format(fname)
    writer = csv.writer(response)
    for hint in (
