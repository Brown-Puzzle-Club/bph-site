...
class UserRegistrationSerializer(serializers.Serializer):
    team_id = serializers.CharField(
        min_length=2,
        error_messages={"min_length": "Team username must be at least 2 characters."},
    )
    ...
    password = serializers.CharField(
        min_length=6,
        error_messages={"min_length": "Team password must be at least 6 characters."},
    )
    ...
    def validate(self, data):
        ...
        if (
            data["in_person"]
            and not data.get("classroom_need", True)
            and data.get("where_to_find", "") == ""
        ):
            raise serializers.ValidationError(
                "Required for in person teams that don't need a room reserved.",
                code="missing_where_to_find",
            )
