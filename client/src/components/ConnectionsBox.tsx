...
const ConnectionsBox: React.FC<ConnectionsBoxProps> = ({ word, isSelected, onClick }) => {
    ...
    const boxStyle = word.includes("N/A") ? (isSelected ? "bg-slate-700 text-white" : "bg-gray-500") : (isSelected ? "bg-slate-700 text-white" : "bg-gray-200");
    return (
    <div
      className={`${boxStyle} h-20 flex items-center justify-center rounded-lg cursor-pointer`}
      onClick={onClick}
    >
      {displayWord}
    </div>
