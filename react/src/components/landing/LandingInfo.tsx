...
const SVG_FILTER = "invert(100%) sepia(0%) saturate(0%) hue-rotate(158deg) brightness(104%) contrast(102%)"
    ...
    <div className='landinginfo pt-5'
      ...
      <div className="grid grid-cols-1 md:grid-cols-9 gap-4 p-5 pb-10">
        <div className="what-box col-start-1 col-span-5 md:col-start-2 md:col-span-3 text-center dark bg-gradient-to-b from-muted/50 to-muted p-6 no-underline outline-none focus:shadow-md btn-gradient-1 relative"
            onMouseEnter={() => { if (HOVER_EFFECT_ACTIVE) (document.getElementsByClassName("blueno_when")[0] as HTMLElement).style.filter = "none"; }}
            onMouseLeave={() => { (document.getElementsByClassName("blueno_when")[0] as HTMLElement).style.filter = SVG_FILTER;}}
        >
          <div className="absolute" style={{top: "-30px",left: "-19px",}}>  
            <img src={blueno_when} className="blueno_when transition ease-in-out duration-200 h-28 w-28 opacity-75" style={{filter: SVG_FILTER,}} 
              />
          </div>
        ...
        <div className="when-box col-start-1 col-span-5 md:col-start-6 md:col-span-3 text-center dark bg-gradient-to-b from-muted/50 to-muted p-6 no-underline outline-none focus:shadow-md btn-gradient-1 relative"
                    onMouseEnter={() => { if (HOVER_EFFECT_ACTIVE) (document.getElementsByClassName("blueno_what")[0] as HTMLElement).style.filter = "none"; }}
                    onMouseLeave={() => { (document.getElementsByClassName("blueno_what")[0] as HTMLElement).style.filter = SVG_FILTER;}}>
          <div className="absolute" style={{top: "-30px",right: "-30px",}}>
            <img src={blueno_what} alt="" className="blueno_what transition ease-in-out duration-200 h-32 w-32 opacity-75" style={{filter: SVG_FILTER,}}
            />
          </div>
