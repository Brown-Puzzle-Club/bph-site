  ...
  return (
    <div className="contact bg-slate-900 text-white h-[90vh] overscroll-contain overflow-hidden overflow-y-auto ">
      ...
      <div className="tabs flex items-center justify-end gap-4 mx-[5%] md:mx-[20%] pr-10 z-10">
          <button onClick={() => setTab(LeaderboardTab.IN_PERSON)} className={`select-none rounded-t-md transition-colors py-2 px-6 hover:text-white ${curTab === LeaderboardTab.REMOTE ? 'hover:bg-[#b3957c]' : ''} ${curTab === LeaderboardTab.IN_PERSON ? 'bg-[#957a62] text-white font-bold' : 'bg-[#745a45] text-[#ffffffb8]'}`}>On-Campus</button>
        ...
        <div className="contact-content custom-scroll h-full max-h-[65dvh] overflow-y-auto">
          {teams.length > 0 ? collectTeams(teams, curTab).map((cur_team, index, array) => (
            <div key={cur_team.id} className={`team-box px-6 pt-3 pb-3 flex items-center space-x-4 text-slate-800 ${index !== array.length - 1 ? 'border-b-4 border-[#957a62]' : ''} ${cur_team.id === team?.id ? 'bg-[#ceaa8a]' : ''}`}>
              ...
              <a className="truncate text-lg w-[80%]" href={`/team/${cur_team.id}`}>{cur_team.team_name}</a>
            </div>
