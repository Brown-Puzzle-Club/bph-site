...
const isMobileDevice = /iphone|ipad|ipod|android|blackberry|windows phone|iemobile|webos/i.test(userAgent);
    ...
    <div className="landing bg-[#02031d] text-white"> 
      {!visitedBefore && isMobileDevice ? (
          <div className="alert z-40 backdrop-blur-sm h-screen w-screen flex items-center fixed justify-center">
            ...
            <button onClick={dismissAlert} className="rounded-md px-6 py-2 font-medium transition-colors hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground focus:outline-none disabled:pointer-events-none disabled:opacity-50 data-[active]:bg-accent/50 data-[state=open]:bg-accent/50">Dismiss</button>
            </div>
          </div>
        ) : null}
      <LandingSplash />
