...
export default function TeamPage() {
  ...
  return (
    <div className="teampage text-center text-white min-h-[90vh]">
      {IS_OWN_TEAM ? <a href={`/my-team`} className="dark bg-muted/20 hover:bg-muted/80 btn-gradient-1 flex font-semibold items-center text-center justify-center my-5 mx-[33%] transition-colors duration-300 group">
              <FaCog className="w-5 h-5 text-slate-400 group-hover:text-white transition-colors duration-300"/>
              <p className="px-2 text-slate-400 group-hover:text-white transition-colors duration-300">Edit Team Info</p>
              <FaCog className="w-5 h-5 text-slate-400 group-hover:text-white transition-colors duration-300"/>
          </a> : null}
      { loadingData ? <BeatLoader className="justify-center content-center pr-2 pt-3" color={'#fff'} size={12} /> : 
        <div className="teampage-content pt-6">
          <TeamIcon className="w-24 h-24 mx-auto text-center" color={cur_team?.color_choice ?? ""} emoji={cur_team?.emoji_choice ?? ""} emoji_cn="text-6xl"/>
